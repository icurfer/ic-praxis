...
규칙은 praxis가 소유하고, 에이전트는 진입점만 제공한다. 강제 계층(git 훅)은
구조적으로 에이전트 중립이다 — *무엇이* 스테이징했든 커밋 시점에 발동한다.
헌법은 마커로 구획된 공유 규칙 블록 하나를 담은 두 개의 네이티브 진입점으로
배포되며, **두 블록이 어긋나면 Gate E가 커밋을 차단**한다 — Claude Code와
Codex를 오가며 써도 규칙이 갈라지지 않는다.

| 레이어 | Claude Code | Codex (`AGENTS.md` 리더) |
|---|---|---|
...
flowchart LR
    ...
    S --> G4{"CLAUDE.md ↔ AGENTS.md<br/>규칙 drift?"}
    G1 -- 예 --> X["🚫 커밋 차단<br/>+ 이유 출력"]
    G2 -- 예 --> X
    G3 -- 예 --> X
    G4 -- 예 --> X
    G1 & G2 & G3 & G4 -- 모두 통과 --> OK["✅ 커밋 진행"]
...
ic-praxis가 규율하는 것은 **프로세스** — 변경이 커밋되기 전에 무엇을 갖춰야 하는가다.
널리 쓰이는 룰셋 상당수는 **코드**를 규율한다(미니멀리즘 사다리, 스타일 가이드, 리뷰 휴리스틱).
둘은 직교하고 함께 쓰기 좋다. 실제로 부딪히는 지점은 헌법 파일 하나뿐이다. 분리하는 방법
세 가지, 좋은 순서대로:

1. **플러그인으로 설치한다.** 대부분의 룰셋은 Claude Code / Codex용 플러그인을 제공한다.
   플러그인은 에이전트 자체 레이어에 살고 레포를 건드리지 않으므로 부딪힐 것이 없다.
   파일 복사 안내는 보통 플러그인 체계가 없는 에이전트를 위한 폴백이다 — 먼저 손대지 말 것.
2. **전역 헌법에 둔다.** `AGENTS.md`를 읽는 에이전트는 전역 파일도 읽는다(Codex는
   `~/.codex/AGENTS.md`). *모든* 프로젝트에 적용하고 싶은 코드 레벨 룰셋이라면 애초에 거기가
   제자리이고, 프로젝트의 `AGENTS.md`는 그대로 남는다.
3. **마커 아래에 덧붙인다.** 꼭 이 레포에 넣어야 한다면 **`<!-- praxis:shared:end -->` 뒤에**
   별도 섹션으로 추가한다. 그 아래는 자유 영역이다 — 배포되는 `AGENTS.md`도 자기 섹션들을
   거기에 두고 있다. 덮어쓰지 말고 덧붙일 것.

**하지 말아야 할 것은 `CLAUDE.md`나 `AGENTS.md`를 다른 프로젝트의 파일로 통째로 바꾸는
것이다** — 헌법이 조용히 사라진다. Gate E가 이것을 잡는다: 한쪽 진입점만 `praxis:shared`
블록을 갖고 있거나 두 블록이 어긋나면 이유를 출력하며 커밋을 막는다.
